...
class LZ96(object):
    ...
    This class models the multi-scale model by Edward Lorenz as in [1]. The
    model consists of the coupled ODEs
    ...
    The state vector x is the collection of all variables X_k and Y_jk are
    ordered X_1, X_2, ..., X_K, Y_11, Y_21, ..., Y_J1, Y_12, ........, Y_JK.
    ...
    """
    def __init__(self, b=10.0, c=10.0, F=8.0, h=1.0, J=10, K=40, dt=0.005):
        """Initializes model parameters.

        Initializes the parameters for the Lorenz 96 model. In [1], he sets
        b = c = 10.0, h = 1.0, K = 36, and J is set to 10, but in [2], F is
        set to 8.0 and K (called J there) equals 40.
        """
    ...
    def model(self, x): # x is the collection of all the x and y variables
        ...
        """
        return fortran.lz96_model(x, self.b, self.c, self.F, self.h, self.J, \
                                 self.K)
    ...
    def model_tlm(self, x, xd):
        ...
        """
        return fortran.lz96_model_tlm(x, xd, self.b, self.c, self.F, self.h, \
                                     self.J, self.K)
    ...
    def model_adj(self, x, yb):
        ...
        """
        return fortran.lz96_model_adj(x, yb, self.b, self.c, self.F, self.h, \
                                     self.J, self.K)
    ...
    def forecast(self, x, ndt=1):
        ...
        for i in xrange(1, ndt):
            y = fortran.lz96_forecast(y, self.b, self.c, self.F, self.h, \
                                     self.J, self.K, self.dt)
    ...
    def forecast_tlm(self, x, xd, ndt=1):
        ...
        """
        yd = fortran.lz96_forecast_tlm(x, xd, self.b, self.c, self.F, self.h, \
                                 self.J, self.K, self.dt)

        for i in xrange(1, ndt):
            x = fortran.lz96_forecast(x, self.b, self.c, self.F, self.h, \
                                 self.J, self.K, self.dt)
            yd = fortran.lz96_forecast_tlm(x, yd, self.b, self.c, self.F, \
    ...
    def forecast_adj(self, x, yb, ndt=1):
        ...
        # Forward sweep
        for i in xrange(1, ndt):
            xlist[i] = fortran.lz96_forecast(xlist[i - 1], self.b, self.c, \
                           self.F, self.h, self.J, self.K, self.dt)

        # Backward sweep
        for i in xrange(ndt - 1, -1, -1):
            xb = fortran.lz96_forecast_adj(xlist[i], xb, self.b, self.c, \
                     self.F, self.h, self.J, self.K, self.dt)
